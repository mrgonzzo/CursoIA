...
Versión Final - Con tabla comparativa y sin guardar gráficos, solo visualización
"""
...
for p in p_range:
    for d in d_range:
        for q in q_range:
            try:
                # Usamos el set de entrenamiento
...
# ============================================================================
# PASO 9.1: TABLA COMPARATIVA A LO LARGO DE LA SERIE
# ============================================================================
print("\n" + "=" * 80)
print("PASO 9.1: TABLA COMPARATIVA (REAL vs AJUSTADO vs PREDICHO)")
print("=" * 80)

documento.append("TABLA COMPARATIVA: VALORES REALES vs. AJUSTADOS/PREDICHOS")
documento.append("-" * 80)

# 1. Crear la serie de valores ajustados (fitted) para el set de entrenamiento
fitted_train = mejor_modelo.fittedvalues.rename('Ajustado (In-Sample)')

# 2. Crear la serie de predicciones para el set de prueba
predictions_test_full = pd.Series(predicciones_test.values, index=test.index).rename('Predicción (Out-Sample)')

# 3. Combinar las series en un solo DataFrame
...
# Llenar la columna 'Ajustado' con los valores del set de entrenamiento
df_comparacion.loc[train.index, 'Ajustado'] = fitted_train.values

# Llenar la columna 'Predicho' con los valores del set de prueba
df_comparacion.loc[test.index, 'Predicho'] = predictions_test_full.values

# Formato para el documento (mostrar solo una muestra para no sobrecargar el informe)
df_print = df_comparacion.copy()
df_print.index = df_comparacion.index.strftime('%Y-%m')

# Mostrar los primeros 5 y últimos 5 registros
print_rows = pd.concat([df_print.head(5), df_print.tail(5)])
# Formatear números para la impresión
print_rows_formatted = print_rows.apply(lambda x: x.map(lambda y: f"{y:.2f}" if pd.notna(y) else '---'))

print("\nMuestra de la serie completa (Valores en toneladas):")
print(print_rows_formatted.to_string(justify='center'))

documento.append("Muestra de la serie completa (Valores en toneladas):")
documento.append("")
documento.append("Nota: 'Ajustado' aplica al set de entrenamiento; 'Predicho' aplica al set de prueba.")
documento.append("")
documento.append(print_rows_formatted.to_string(justify='center'))
documento.append("")
# ============================================================================
# FIN PASO 9.1
# ============================================================================
