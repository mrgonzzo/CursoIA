...
def calcular_coeficiente_theil(actual, prediccion):
    ...
    u_m = (mean_pred - mean_actual) ** 2
    u_s = (np.std(prediccion) - np.std(actual)) ** 2
    u_c = 2 * (1 - np.corrcoef(actual, prediccion)[0, 1]) * np.std(prediccion) * np.std(actual)

    total_u = u_m + u_s + u_c

    bias_proportion = u_m / total_u if total_u != 0 else 0
    variance_proportion = u_s / total_u if total_u != 0 else 0
    covariance_proportion = u_c / total_u if total_u != 0 else 0
    ...
    for d in d_range:
        for q in q_range:
            try:
                ...
                modelo_fit = modelo.fit(disp=False)  # disp=False para evitar output excesivo
...
# ============================================================================
# PASO 9: PREDICCIONES Y COEFICIENTE DE THEIL + TABLAS COMPARATIVAS
# ============================================================================
print("\n" + "=" * 80)
...
# ----------------------------------------------------------------------------
# 9.1 EVALUACIÓN IN-SAMPLE (Ajustado) y OUT-OF-SAMPLE (Predicho - Test)
# ----------------------------------------------------------------------------

# 1. Ajuste In-Sample (Últimos meses de entrenamiento)
fitted_values_train = mejor_modelo.fittedvalues
n_fit_show = 12  # Mostrar los últimos 12 meses de ajuste

# 2. Predicciones Out-of-Sample (Conjunto de prueba)
predicciones_test = mejor_modelo.forecast(steps=len(test))

# Crear DataFrame de Comparación para el Conjunto de Prueba
df_comparacion_test = pd.DataFrame({
    'Fecha': test.index.strftime('%Y-%m'),
    'Real (y_t)': test.values.round(2),
    'Predicho (ŷ_t)': predicciones_test.values.round(2),
    'Error (e_t)': (test.values - predicciones_test.values).round(2),
})

documento.append("TABLA 1: COMPARATIVA REAL vs. PREDICHO (Conjunto de Prueba)")
documento.append("  Período: " + df_comparacion_test['Fecha'].iloc[0] + " a " + df_comparacion_test['Fecha'].iloc[-1])
documento.append("-" * 80)
# Usar to_string para formatear la tabla sin el índice de Pandas
documento.append(df_comparacion_test.to_string(index=False))
documento.append("-" * 80)
documento.append("")

# ----------------------------------------------------------------------------
# 9.2 Métricas y Theil
# ----------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------
# 9.3 Predicción Futura (Forecast) y su tabla
# ----------------------------------------------------------------------------
...
# Crear DataFrame de Predicción Futura
df_forecast_futuro = pd.DataFrame({
    'Fecha': forecast_index.strftime('%Y-%m'),
    'Predicción': forecast.round(2),
    'IC 95% Inferior': forecast_ci.iloc[:, 0].round(2),
    'IC 95% Superior': forecast_ci.iloc[:, 1].round(2),
})

documento.append(f"TABLA 2: PREDICCIONES FUTURAS ({n_forecast} meses adelante)")
documento.append("  Período: " + df_forecast_futuro['Fecha'].iloc[0] + " a " + df_forecast_futuro['Fecha'].iloc[-1])
documento.append("-" * 80)
documento.append(df_forecast_futuro.to_string(index=False))
documento.append("-" * 80)
documento.append("")
