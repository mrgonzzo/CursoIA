...
# --- MODELOS PARA CADA VARIABLE ---
model_dict = {}
for target in targets:
    y = df[target]
    model = sm.OLS(y, X).fit()
    model_dict[target] = model
    print(f"=== RESUMEN MODELO {target} ===")
    print(model.summary())

    # Efectos marginales
    print(f"\n=== EFECTOS MARGINALES {target} ===")
    for var in features:
        coef = model.params[var]
        pval = model.pvalues[var]
        print(f"{var}: coef={coef:.3f}, p-value={pval:.4f}")
...
if os.path.exists(csv_real):
    ...
    for target, model in model_dict.items():
        df_real[f"{target}_pred"] = model.predict(X_real)
    df_real.to_csv(os.path.join(output_path, "uci_fallecidos_predicciones.csv"), index=False)
    print("✅ Predicciones guardadas en 'uci_fallecidos_predicciones.csv'")
