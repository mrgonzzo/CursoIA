...
if os.path.exists(csv_real):
    ...
    for target, model in model_dict.items():
        ...
        df_real[f"{target}_pred_CI_lower"] = pred_real.conf_int()[:, 0]
        df_real[f"{target}_pred_CI_upper"] = pred_real.conf_int()[:, 1]
    df_real.to_csv(os.path.join(output_path, "uci_fallecidos_predicciones_avanzado.csv"), index=False)
    print("✅ Predicciones guardadas en 'uci_fallecidos_predicciones_avanzado.csv'")
